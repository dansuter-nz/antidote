...
else 
{
	if ($_GET["n"]=="") 
	{
		...
		<form action="crowd.php" method="get">
			...
			<div class="row" style="padding-top: 10px;">
				...
				{
					?>	
				<div class="col-xs-12 col-sm-12 alignRight" >
					Filter by categories <select  class="dropfilter pl10" id="f" name="f" onchange="filter(this.value,'','')">
					<?php
					$h=$h.'<option value="no_filter">No Filter</option>';
		...
		if (mysqli_num_rows($result)>0) {
			...
			while($row = $result->fetch_assoc())
				...
				if ($irow % 2==1) 
			    	...
			    	{ $strClass="white_row";}				
				$h=$h.'<tr class="'.$strClass.'"> <td><a href="/projects/wisdom/crowd.php?a='.$row["ID"].'">'.$row["streamTitle"]."</a></td>";
	...
	else 
	{
		...
		if (mysqli_num_rows($result)>0) 
			...
			$h=$h."<h2>In total ".$row["Total_Comments"].' comments. <span class="green"> total upvotes '.$row["Total_posVotes"].'</span> -  total down votes <span class="red">'.$row["Total_negVotes"].'</span>.<br> (Overall Rank <a title="show rankings" href="/projects/wisdom/crowdranking.php">#'.$row["Overall_Rank"]." of ".$row["Total_people"].')</a> people who have commented.  So what is "'.$_GET["n"].'" saying?</h2>';
			//check to see if login is admin?
			if ($_SESSION["can_authorize"]) 
				{
					$h=$h.'<form id="myForm" action="/projects/wisdom/crowd.php" method="post"><input type="hidden" name="a" id="a" value="'.$article.'"><div class="row"><div class="col-sm-11"><textarea cols="125" rows="8" name="ac" id="ac">'.$row["Antidote_comment"].'</textarea></div><div class="col-sm-1"> <input type="button" id="updateAdminComment" value="Update" onclick="updateComment()"></div></form>';
				...
				if (!is_null($row["Antidote_comment"]))
					{
			...
			while($row = $result->fetch_assoc())
				...
				if ($irow % 2==1) 
		    		...
		    		{$strClass="white_row";}
		    	$irow=$irow+1;
				$h=$h.'<tr class="'.$strClass.'"><td colspan="5"><a href="/projects/wisdom/crowd.php?a='.$row["ID"].'">'.$row["streamTitle"]."</td></tr>";
				$h=$h.'<tr class="'.$strClass.'"><td>'.$row["commentText"].'</td>';
				$h=$h.'<td class="tdRight bold pr10 pl10"> '.$row["TotalVotes"]." </td>";
				$h=$h.'<td class="tdRight bold green pr10 pl10"> '.$row["posVotes"]." </td>";
				$h=$h.'<td class="tdRight bold red pr10 pl10"> '.$row["negVotes"].' </td>';
				//$sSQL="call adds.get_rank_person_article('".$row["name"]."','".$row["streamID"]."',".$row["TotalVotes"].")";
				//$result = $conn2->query($sSQL) or die($conn->error);
				//echo $sSQL."<br>";
				//while($row = $result->fetch_assoc())
					//{
					//$irank=$rowA[0];
					...
					//$h=$h.'<td class="bold">'.$irank."</a>/".$row["commentCount"]."</td>
				$h=$h.'</tr>';		
				}
			$h=$h."</table>";
...
if (f1!="")
	{		
		if (url.indexOf("&f1=")>0)
			{url=url.substring(0,url.indexOf("&f1="))}
		if (f1!="no_filter")
		{
		//remove all filters
		url=url+"&f1="+f1;
...
if (f2!="")
	{if (url.indexOf("&f2=")>0)
		{url=url.substring(0,url.indexOf("&f2="))}
	if (f2!="no_filter")
		{
		//remove all filters
		url=url+"&f2="+f2;
